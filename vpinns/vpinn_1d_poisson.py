...
Otimizacao: L-BFGS (jaxopt.LBFGS), no mesmo padrao usado em
svpinn_1d_poisson.py (fori_loop de eval_freq passos de L-BFGS dentro de
um lax.scan de num_steps // eval_freq blocos), substituindo o Adam.
...
# ==========================================
def train_lbfgs(params, loss_fn, num_steps, eval_freq, X_test, Y_test,
    ...
    X_test/Y_test) a cada `eval_freq` passos.

    Mesmo padrao em dois niveis usado em `svpinn_1d_poisson.py`: um
    `jax.lax.fori_loop` de `eval_freq` passos de L-BFGS (sem avaliacao)
    dentro de um `jax.lax.scan` de `num_steps // eval_freq` blocos (uma
    avaliacao por bloco). `loss_fn` ja fecha sobre os dados fixos da
    formulacao variacional (x_q, w_q_phys, V/Vx_phys, F, tau, g, h), por
    isso o objetivo do L-BFGS depende apenas do vetor de parametros
    achatado.
    """
    ...
    def block_fn(carry, _):
        ...
        Y_nn = forward(X_test, unflatten_fn(p_next))
        ...
        for method_tag, loss_builder in methods.items():
            ...
            for run in range(NUM_RUNS):
                ...
                params, loss_traj, err_traj = train_lbfgs(
                    params, loss_builder, NUM_STEPS, EVAL_FREQ, X_TEST, Y_TEST
                )
